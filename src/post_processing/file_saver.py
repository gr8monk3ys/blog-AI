...
def save_to_file(content: str, options: SaveOptions) -> str:
    """
...
def save_blog_post(
    ...
    try:
        ...
        save_options = SaveOptions(
            file_path=file_path, format=format, overwrite=overwrite
        )
    ...
    try:
        ...
        if head_start != -1 and head_end != -1:
            ...
            for key, value in metadata.items():
                if key == "title":
                    ...
                    if title_start != -1 and title_end != -1:
                        ...
                        head_content = (
                            head_content[: title_start + 7]
                            + value
                            + head_content[title_end:]
                        )
                    else:
                        # Add title tag
            ...
            if html_start != -1:
                ...
                content = (
                    content[: html_start + 6] + head_tag + content[html_start + 6 :]
                )
            else:
