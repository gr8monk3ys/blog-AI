...
def create_repository(
    ...
    try:
        ...
        repo_data = {"name": name, "private": private}
        ...
        if response.status_code in (200, 201):
            ...
            return GitHubRepository(
                owner=repo_data["owner"]["login"], name=repo_data["name"]
            )
        else:
...
def get_repository(
    credentials: GitHubCredentials, owner: str, name: str
) -> GitHubRepository:
    """
...
def create_pull_request(
    ...
    try:
        ...
        pr_data = {"title": title, "head": head, "base": base}
...
def upload_blog_post(
    ...
    try:
        ...
        # Create file options
        options = GitHubFileOptions(
            path=path, content=content, message=commit_message, branch=branch
        )
