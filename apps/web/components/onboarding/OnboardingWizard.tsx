...
function useOnboardingWizardView({
  ...
  const buildActivationHref = useCallback((): string => {
    const params = new URLSearchParams()

    if (formData.topicSuggestion.trim()) {
      params.set('topic', formData.topicSuggestion.trim())
    }
    if (formData.toneKeywords.length > 0) {
      params.set('tone', formData.toneKeywords[0] ?? formData.writingStyle)
    } else {
      params.set('tone', formData.writingStyle)
    }
    params.set('source', 'onboarding')
    params.set('contentType', formData.firstContentType)

    return `/bulk?${params.toString()}`
  }, [formData.firstContentType, formData.topicSuggestion, formData.toneKeywords, formData.writingStyle])
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 max-w-md mx-auto">
          ...
          workflow so you can start with momentum.
        </p>
      </div>
    ...
    <div className="space-y-8 text-center">
      <div>
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 max-w-md mx-auto">
          ...
          batch using your saved voice and the topic you just chose.
        </p>
