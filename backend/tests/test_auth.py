...
class TestIsProduction(unittest.TestCase):
    ...
    def test_is_production_case_insensitive(self):
        """_is_production() should handle case-insensitive comparison."""
        with patch.dict(os.environ, {
            "SENTRY_ENVIRONMENT": "PRODUCTION",
        }, clear=True):
            auth_module = get_auth_module()
...
class TestVerifyAPIKeyDependency(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_verify_api_key_dependency_returns_dev_user_with_dev_api_key(self):
    ...
    async def test_verify_api_key_dependency_blocks_dev_api_key_in_production(self):
