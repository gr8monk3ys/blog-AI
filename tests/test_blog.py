...
class TestBlogModule(unittest.TestCase):
        ...
        self.sample_blog_post = BlogPost(
            ...
            sections=[
                Section(
                    title="Introduction",
                    subtopics=[
                        SubTopic(title="", content="This is the introduction content.")
                    ],
                ),
                Section(
                    title="Main Section",
                    subtopics=[
                        SubTopic(
                            title="Subtopic 1", content="This is subtopic 1 content."
                        ),
                        SubTopic(
                            title="Subtopic 2", content="This is subtopic 2 content."
                        ),
                    ],
                ),
                Section(
                    title="Conclusion",
                    subtopics=[
                        SubTopic(title="", content="This is the conclusion content.")
                    ],
                ),
            ],
            tags=["test", "blog"],
    ...
    def test_generate_blog_post(
        ...
        mock_section.return_value = MagicMock(
            title="Section 1", subtopics=[MagicMock(content="Section content")]
        )
    ...
    @patch("src.blog.make_blog.generate_meta_description")
    def test_generate_blog_post_with_research(
        self,
        mock_meta,
        mock_faqs,
        mock_conclusion,
        ...
        mock_intro.return_value = MagicMock(
            title="Introduction", subtopics=[MagicMock(content="Intro content")]
        )
        mock_section.return_value = MagicMock(
            title="Section 1", subtopics=[MagicMock(content="Section content")]
        )
        mock_conclusion.return_value = MagicMock(
            title="Conclusion", subtopics=[MagicMock(content="Conclusion content")]
        )
        mock_faqs.return_value = MagicMock(
            title="FAQ", subtopics=[MagicMock(title="Q1", content="A1")]
        )
        mock_meta.return_value = MagicMock(content="Meta description")
    ...
    def test_post_process_blog_post(self, mock_humanize, mock_proofread):
        ...
        result = post_process_blog_post(
            blog_post=self.sample_blog_post, proofread=True, humanize=True
        )
    ...
    def test_save_and_load_blog_post(self):
        ...
        try:
            ...
            self.assertEqual(
                len(loaded_post.sections), len(self.sample_blog_post.sections)
            )
