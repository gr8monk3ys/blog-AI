...
class TestBookModule(unittest.TestCase):
        ...
        self.sample_book = Book(
            ...
            chapters=[
                ...
                Chapter(
                    ...
                    title="Chapter 1",
                    topics=[
                        Topic(title="Introduction", content="Chapter 1 introduction."),
                        Topic(title="Section 1", content="This is section 1 content."),
                        Topic(title="Conclusion", content="Chapter 1 conclusion."),
                    ],
                ),
                Chapter(
                    number=2,
    ...
    def test_generate_book(
        self,
        mock_conclusion_chapter,
        mock_intro_chapter,
        mock_chapter,
        mock_clusters,
        mock_provider,
        ...
        mock_clusters.return_value = [
            MagicMock(
                main_topic="Chapter 1",
                subtopics=["Section 1", "Section 2"],
                keywords=["key1", "key2"],
            )
        ]
        mock_chapter.return_value = MagicMock(title="Chapter 1", topics=[MagicMock()])
        mock_intro_chapter.return_value = MagicMock(
            title="Introduction", topics=[MagicMock()]
        )
        mock_conclusion_chapter.return_value = MagicMock(
            title="Conclusion", topics=[MagicMock()]
        )
    ...
    @patch("src.book.make_book.generate_conclusion_chapter")
    def test_generate_book_with_research(
        self,
        mock_conclusion,
        mock_intro,
        mock_chapter,
        ...
        mock_conclusion.return_value = MagicMock(
            title="Conclusion", topics=[MagicMock()]
        )
    ...
    def test_generate_chapter(self, mock_conclusion, mock_section, mock_intro):
        ...
        mock_intro.return_value = MagicMock(
            title="Introduction", subtopics=[MagicMock()]
        )
        mock_section.return_value = MagicMock(
            title="Section 1", subtopics=[MagicMock()]
        )
        mock_conclusion.return_value = MagicMock(
            title="Conclusion", subtopics=[MagicMock()]
        )
    ...
    def test_generate_introduction_chapter(self, mock_generate_text):
        ...
        chapters = [MagicMock(title="Chapter 1"), MagicMock(title="Chapter 2")]

        # Call the function
        result = generate_introduction_chapter(
            title="Test Book", chapters=chapters, keywords=["test", "book"]
        )
    ...
    def test_post_process_book(self, mock_humanize, mock_proofread):
        ...
        # Call the function
        result = post_process_book(book=self.sample_book, proofread=True, humanize=True)
        ...
        topic_count = sum(len(chapter.topics) for chapter in self.sample_book.chapters)
