...
class TestContentCalendar(unittest.TestCase):
    ...
    def test_generate_content_calendar(self, mock_generate_topics):
        ...
        mock_generate_topics.return_value = [
            ContentTopic(
                title="Topic 1", keywords=["key1", "key2"], description="Description 1"
            ),
            ContentTopic(
                title="Topic 2", keywords=["key3", "key4"], description="Description 2"
            ),
        ]

        # Call the function
        result = generate_content_calendar(
            niche="test niche", timeframe="month", content_types=["blog"], frequency=7
        )
    ...
    def test_generate_content_topics(self, mock_generate_text):
        ...
        # Call the function
        result = generate_content_topics(niche="test niche", num_topics=2)
    ...
    def test_save_and_load_content_calendar(self):
        ...
        try:
            ...
            # Assertions
            self.assertEqual(
                len(loaded_calendar.items), len(self.sample_calendar.items)
            )
            self.assertEqual(
                loaded_calendar.items[0].topic.title,
                self.sample_calendar.items[0].topic.title,
            )
            self.assertEqual(
                loaded_calendar.items[1].topic.title,
                self.sample_calendar.items[1].topic.title,
            )
            self.assertEqual(
                loaded_calendar.start_date.strftime("%Y-%m-%d"),
                self.sample_calendar.start_date.strftime("%Y-%m-%d"),
            )
            self.assertEqual(
                loaded_calendar.end_date.strftime("%Y-%m-%d"),
...
class TestCompetitorAnalysis(unittest.TestCase):
    ...
    def test_analyze_competitors(
        self, mock_recommendations, mock_gaps, mock_keywords, mock_content, mock_website
    ):
        ...
        # Call the function
        result = analyze_competitors(niche="test niche", competitors=["Competitor 1"])
...
class TestTopicClusters(unittest.TestCase):
    ...
    def test_generate_topic_clusters(self, mock_generate_text):
        ...
        result = generate_topic_clusters(
            niche="test niche", num_clusters=2, subtopics_per_cluster=3
        )
        ...
        self.assertEqual(
            result[0].subtopics, ["Subtopic 1.1", "Subtopic 1.2", "Subtopic 1.3"]
        )
        self.assertEqual(result[0].keywords, ["key1", "key2", "key3"])
        self.assertEqual(result[1].main_topic, "Main Topic 2")
        self.assertEqual(
            result[1].subtopics, ["Subtopic 2.1", "Subtopic 2.2", "Subtopic 2.3"]
        )
        self.assertEqual(result[1].keywords, ["key4", "key5", "key6"])
...
class TestContentOutline(unittest.TestCase):
    ...
    def test_generate_content_outline(self, mock_generate_text):
        ...
        result = generate_content_outline(
            title="Test Outline", keywords=["key1", "key2", "key3"], num_sections=2
        )

        # Assertions
        self.assertEqual(result.title, "Test Outline")
        self.assertEqual(
            result.sections, ["Introduction", "Section 1", "Section 2", "Conclusion"]
        )
    ...
    def test_generate_detailed_content_outline(self, mock_generate_text):
        ...
        result = generate_detailed_content_outline(
            title="Test Outline", keywords=["key1", "key2", "key3"], num_sections=1
        )
    ...
    @patch("src.planning.content_outline.generate_text")
    def test_generate_content_outline_with_research(
        self, mock_generate_text, mock_research
    ):
